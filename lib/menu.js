...
Menu.prototype.handleInput = function(input, cb) {
  ...
  const navigate = function( cb ) {
    if( option.navigate ) {
      ...
      if( that._navigate[option.navigate] !== undefined ) {
        that._navigate[option.navigate]( cb );
      } else {
        that._navigate['[navigate]'](option.navigate, cb );
      }
  ...
  if( option.action ) {
    if( this._actions[option.action] ) {
      this.executeAction( option.action, option.actionParams, function() {
        navigate(function(err) {
          cb( err, true);
        });
      } );
    }
