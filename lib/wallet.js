...
Wallet.prototype.readMnemonic = function( wordIndex, readMnemonicFinished ) {
  ...
  if( wordIndex < 0 || wordIndex >= 12 ) {
    return readMnemonicFinished(new Error("Index out of range"));
  }


  const word = that.mnemonic[wordIndex];

  const toSpeak = [this._texts[wordIndex], word].concat(word.split('')).concat([word]);

  this._output.writeln( toSpeak, readMnemonicFinished );
